	...
	angular
		...
		.controller('firstCtrl', function(
			...
			function initCurrentMenu() {
				$rootScope.detailThirdThirdMenu = 1
			}
			function initList() {
				$scope.imageList = []
				$scope.wordList = []
			...
			function initDutyList() {
				getDutyList(
					global.projectId,
					handleGetDutyList,
					handleGetDutyListError
				)
			}
			function getDutyList(projectId, handle, handleError) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.getDutyList(projectId)
					.then(handle)
					.catch(handleError)
			}

			function handleGetDutyList(res) {
				console.log('res', res)
			...
			function handleGetDutyListError(error) {
				console.log(error)

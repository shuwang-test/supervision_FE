	...
	angular
		...
		.controller('secondCtrl', function(
			...
			function initCurrentMenu() {
				$rootScope.detailThirdThirdMenu = 2
			}
			function initList() {
				$scope.imageList = []
				$scope.wordList = []
			...
			function initDutyDetailList() {
				getDutyDetailList(
					global.projectId,
					handleGetDutyDetailList,
					handleGetDutyDetailListError
				)
			}
			function getDutyDetailList(projectId, handle, handleError) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.getDutyDetailList(projectId)
					.then(handle)
					.catch(handleError)
			}

			function handleGetDutyDetailList(res) {
				console.log('res', res)
			...
			function handleGetDutyDetailListError(error) {
				console.log(error)
			...
			function deleteImageAndWord() {
				...
				deleteDutyDetail(data)
			}

			function deleteDutyDetail(data) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.deleteDutyDetail(data)
					.then(function(res) {
			...
			function uploadDutyDetail(data) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.uploadDutyDetail(data)
					.then(function(res) {

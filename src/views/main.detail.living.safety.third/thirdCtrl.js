	...
	angular
		...
		.controller('thirdCtrl', function(
			...
			function initCurrentMenu() {
				$rootScope.detailThirdThirdMenu = 3
			}
			function initList() {
				$scope.imageList = []
				$scope.wordList = []
			...
			function initMeasureList() {
				getMeasureList(
					global.projectId,
					handleGetMeasureList,
					handleGetMeasureListError
				)
			}
			function getMeasureList(projectId, handle, handleError) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.getMeasureList(projectId)
					.then(handle)
					.catch(handleError)
			}

			function handleGetMeasureList(res) {
				console.log('res', res)
			...
			function handleGetMeasureListError(error) {
				console.log(error)

...
// 	angular
// 		.module('app')
// 		.controller('constructPermissionCtrl', constructPermissionCtrl)
// 	function constructPermissionCtrl(
// 		$scope,
// 		$rootScope,
// 		$timeout,
// 		$state,
...
;(function() {
	...
	angular
		.module('app')
		.controller('constructPermissionCtrl', function(
			$scope,
			$rootScope,
			toastr,
			Constant,
			...
			init()
			function init() {
				$rootScope.earlyCurrentMenu = 6
				initGlobal()
				initList()
				initDetailSecondMenu()
				initConstructPermissionList()
			}
			function initGlobal() {
			...
			function initConstructPermissionList() {
				getConstructPermissionList(
					global.projectId,
					handleGetConstructPermissionList,
					handleGetConstructPermissionListError
				)
			}
			function getConstructPermissionList(
				projectId,
				handle,
				handleError
			) {
				...
				$httpFactory
					.getConstructPermissionList(projectId)
					.then(handle)
					.catch(handleError)
			}

			function handleGetConstructPermissionList(res) {
				console.log('res', res)
			...
			function handleGetConstructPermissionListError(error) {
				console.log(error)
			...
			function deleteImageAndWord() {
				...
				deleteConstructPermission(data)
			}

			function deleteConstructPermission(data) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.deleteConstructPermission(data)
					.then(function(res) {
					...
					Upload.upload({
						...
						.then(function(res) {
							...
							if (flag) {
								...
								uploadConstructPermission(data)
							}
						})
						.catch(function(error) {
							console.log(error)
			...
			function uploadConstructPermission(data) {
				$httpFactory.setToken(global.token)
				$httpFactory
					.uploadConstructPermission(data)
					.then(function(res) {
